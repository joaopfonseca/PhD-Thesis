...
Research focused on AL has typically been focused on the
...
% Pipelined approach
The only AL model found that uses data augmentation outside of the
...
The proposed AL framework is depicted in Figure~\ref{fig:al_proposed}.
...
The methodology developed serves a two-fold purpose: (1) Compare classification
...
\begin{itemize}
    ...
    \item Area Under the Learning Curve (AULC). It is the sum of the
        ...
        to facilitate the interpretability of this metric.
...
active learner or classifier is trained five times using a different partition as
...
set and 60\% is used as the training set. The AL simulations and the classifiers'
training occur within the training set. However, the classifiers used to find the
...
(\textit{i.e.,} all AL simulations use less than 80\% of the training set). Once
...
The proposed method evidenced more consistent data utilization requirements
